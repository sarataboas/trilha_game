...
class Game {
    ...
    checkWinner(playerQuit = null) {
        ...
        if (playerQuit !== null) {
            ...
            this.updateStatusMessage(`Vencedor por desistência - ${winner}!`);
            this.gameEnd = true;
...
async function startGameVsPlayer(boardSize, nick, password) {
    try {
        ...
        const data = await updatePlayers(gameID, nick);
        console.log("Tabuleiro:", data.board);
        // Configura a lógica do jogo
        // setupGameLogic(data, gameID, nick, boardSize);
...
// // Configura lógica do jogo
// function setupGameLogic(data, gameID, nick,boardSize) {
//     const players = data.players;
//     const playerColor = players[nick];
//     const opponentNick = Object.keys(players).find(p => p !== nick);
//     const opponentColor = players[opponentNick];


//     game = new Game(boardSize, playerColor, opponentColor, false, null, null, nick, opponentNick);
//     console.log("Jogadores configurados:", { nick, opponentNick });
// }
...
function setupBoardEvents(gameID, nick, password) {
    document.getElementById('game-board').addEventListener('click', async (event) => {
        ...
        if (isPoint || isRedPiece || isBluePiece) {
            ...
            const square = Math.floor(row / 3);
            const position = col;

            const cell = { square, position };

            const moveSuccess = await sendMove(nick, password, gameID, cell);
            const serverData = await updatePlayers(gameID,nick);

            if (serverData && serverData.board) {
                updateBoardFromServer(serverData.board); // Atualiza diretamente com o servidor
            }
        }
    });
}
...
function startGameVsAI(boardSize, nick) {
    document.querySelector('.config-area').style.display = 'none';
    document.getElementById('board-area').style.display = 'block';
...
document.getElementById('classifications-button').addEventListener('click', () => openModal('class-modal'));
